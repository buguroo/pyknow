...
def test_or_inside_or():
    ...
    input_ = OR(Fact(a=1), OR(Fact(a=3), Fact(a=4)))
    output = OR(Fact(a=1), Fact(a=3), Fact(a=4))
    assert dnf(input_) == output
...
def test_or_and_fact_inside_or():
    ...
    input_ = OR(Fact(a=1), OR(Fact(a=2), Fact(a=3)))
    output = OR(Fact(a=1), Fact(a=2), Fact(a=3))
    assert dnf(input_) == output


def test_or_inside_or_inside_and():
    from pyknow.rule import OR, AND
    from pyknow.fact import Fact
    from pyknow.rete.network.dnf import dnf

    input_ = AND(Fact(b=1), OR(Fact(a=1), OR(Fact(a=3), Fact(a=4))))
    output = OR(AND(Fact(b=1), Fact(a=1)),
