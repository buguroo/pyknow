...
def get_func_name(method):
    return method[0].__repr__().split('.')[0].split(' ')[1].strip()
...
def test_get_alpha_branch():
    ...
    class FakeFeatureTesterNode(namedtuple("Fake", "value")):
        def add_child(self, child, method):
            nonlocal executions
            executions.append(get_func_name([child.value]))
    ...
    assert [get_func_name([a]) for a in branch] == ["match_L"]
    ...
    input_node_callable = get_func_name([input_node.value])
...
def test_network_generation():
    ...
    def _get_node(node):
        if not isinstance(node, ConflictSetNode):
            yield get_func_name([node.matcher])
        else:
